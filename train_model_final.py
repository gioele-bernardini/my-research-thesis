...
model = Sequential([
    ...
    Flatten(),
    Dropout(0.2),
    # Ridotto da 40 a 20 neuroni per diminuire il numero di parametri in questo layer.
    Dense(20,
          activation='relu',
          kernel_regularizer=regularizers.l2(0.001),
          name='hidden_layer1'),
...
# Predizioni e matrice di confusione sul test set.
predictions = model2.predict(X_test)
predicted_classes = np.argmax(predictions, axis=1)
cm = tf.math.confusion_matrix(Y_test, predicted_classes)
print("Matrice di confusione (test set):\n", cm.numpy())

# --- Fine tuning opzionale sull'intero dataset ---
# FIXME: Imposta la variabile booleana per controllare se effettuare il fine tuning.
do_fine_tuning = False

if do_fine_tuning:
    # Concatena i dati di train, validate e test.
    ...
    print("Fine tuning non eseguito. Utilizzo il modello checkpoint come modello finale.")
